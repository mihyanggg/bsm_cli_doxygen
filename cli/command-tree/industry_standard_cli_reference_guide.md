...
### Parent Commands (Modes) 
```
policy-map <fbs-policy-name> type { qos | monitoring | forwarding | copp | acl-copp }
```
### Syntax 
```
...
```
clear counters service-policy { Switch | CtrlPlane } [ type { qos | monitoring | forwarding | copp | acl-copp } ]
...
```
clear counters service-policy interface { <eth-if-id> | <po-if-id> | <vlan-if-id> | <eth-sub-if-id> | <po-sub-if-id> | CPU } [ type { qos | monitoring | forwarding | copp | acl-copp } ]
...
| vlan-if-id | VlanNUM  |   |
| eth-sub-if-id | EthernetX.Y  |   |
| po-sub-if-id | PortChannelNUM  |   |
...
```
clear counters service-policy policy-map <fbs-policy-name> { { [ interface { <eth-if-id> | <po-if-id> | <vlan-if-id> | <eth-sub-if-id> | <po-sub-if-id> | CPU } ] } | [ Switch ] | [ CtrlPlane ] } ]
...
```
clear ip access-list counters [ <access-list-name> { { [ interface { Ethernet | <PortChannel> | <Vlan> | <eth-sub-if-id> | <po-sub-if-id> } ] } | [ Switch ] ] } ]
...
```
clear ip arp interface { <phy-if-name> | <subif-name> | <mgmt-if-name> | <po-if-name> | <vlan-if-name> }
...
```
clear ipv6 access-list counters [ <access-list-name> { { [ interface { Ethernet | <PortChannel> | <Vlan> | <eth-sub-if-id> | <po-sub-if-id> } ] } | [ Switch ] ] } ]
...
```
clear ipv6 neighbors interface { <phy-if-name> | <subif-name> | <mgmt-if-name> | <po-if-name> | <vlan-if-name> }
...
```
clear mac access-list counters [ <access-list-name> { { [ interface { Ethernet | <PortChannel> | <Vlan> | <eth-sub-if-id> | <po-sub-if-id> } ] } | [ Switch ] ] } ]
...
```
### Examples 
#### Add description to next hop group 
```
...
| Name | Description | Type |
|:---:|:-----:|:-----:|
| ecn | ECN Options  | Select [none(ecn_none) green(ecn_green) ]  |
### Usage Guidelines 
```
...
```
interface <phy-sub-if-name>
interface PortChannel <lag-id-subid>

```
### Syntax 

```
encapsulation dot1q vlan-id <vlanid>
...
### Description 
```
Configured next-hop group member at specified index 
```
### Parent Commands (Modes) 
```
pbf next-hop-group <fbs-nhgrp-name> [ type { ip | ipv6 } ]
...
### Syntax 
```
entry <entry-index> { next-hop <ip-address> { [ vrf { <vrf-name> | default } ] } { [ recursive ] | [ non-recursive ] | [ overlay ] ] } }
no entry <entry-index>
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
...
### Syntax 
```
interface { <phy-if-name> | <vlan-if-name> | <phy-sub-if-name> }
no interface { <vlan_range_num> | <po_range_num> | <phy-sub-if-name> }
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
...
```
interface PortChannel { { <lag-id> { [ mode <PoMode> ] } { [ min-links <min-links-value> ] } [ fallback ] [ fast_rate ] } | <lag-id-subid> }
...
| breakout_mode | Port breakout mode  | Select [1x100G 1x40G 2x100G 2x50G 4x100G 4x25G 4x10G 1x400G 2x200G 4x50G 8x50G 8x25G 8x10G ]  |
### Usage Guidelines 
```
...
### Syntax 
```
interface media-fec port <slotport> mode <media-fec-mode>
no interface media-fec port <slotport>
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
| slotport | Front-panel port (slot/port)  |   |
| media-fec-mode | Media FEC (forward error correction) mode  | Select [ieee custom ]  |
### Usage Guidelines 
```
...
edia-fec on port in-progress, use 'show interface interface-name' to check status.
```
## interface range 
...
## interface transceiver diagnostics 
### Description 
```
Transceiver diagnostics configuration 
```
### Parent Commands (Modes) 
```
configure terminal
...
### Syntax 
```
interface transceiver diagnostics { { loopback { host-side-input | host-side-output | media-side-input | media-side-output } } | { pattern { checker-host | checker-media | generator-host | generator-media } } } [ <phy-if-name> ]
no interface transceiver diagnostics { { loopback { host-side-input | host-side-output | media-side-input | media-side-output } } | { pattern { checker-host | checker-media | generator-host | generator-media } } } [ <phy-if-name> ]
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
...
| Name | Description | Type |
|:---:|:-----:|:-----:|
| intfName | EthernetX.Y  |   |
| pchName | PortChannelNUM  |   |
| vlanName | VlanNUM  |   |
| loName |   |   |
### Usage Guidelines 
```
...
no ip igmp join <mcastgrpaddr> <srcaddr>
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
| mcastgrpaddr | A.B.C.D  | String  |
| srcaddr | A.B.C.D  | String  |
### Usage Guidelines 
```
...
#### Configure IGMP join multicast group 
```
sonic-cli(config)# interface Ethernet 4.1
sonic-cli(conf-if-Ethernet4.1)# ip igmp join mcastgrpaddr srcaddr
```
...
### Syntax 
```
ip igmp last-member-query-count <lmquerycount>
no ip igmp last-member-query-count
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
| lmquerycount | IGMP version 1 to 7(Default: 2)  | Integer  |
### Usage Guidelines 
```
Use this command to configure IGMP.
```
...
```
sonic-cli(config)# interface Ethernet 4.1
sonic-cli(conf-if-Ethernet4.1)# ip igmp last-member-query-count count
```
...
### Syntax 
```
ip igmp last-member-query-interval <lmqueryinterval>
no ip igmp last-member-query-interval
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
| lmqueryinterval | IGMP version 1 to 255(Default: 10)  | Integer  |
### Usage Guidelines 
```
Use this command to configure IGMP.
```
...
```
sonic-cli(config)# interface Ethernet 4.1
sonic-cli(conf-if-Ethernet4.1)# ip igmp last-member-query-interval interval
```
...
### Syntax 
```
ip igmp query-interval <queryinterval>
no ip igmp query-interval
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
| queryinterval | IGMP version 1 to 1024(Default: 125)  | Integer  |
### Usage Guidelines 
```
Use this command to configure IGMP.
```
...
```
sonic-cli(config)# interface Ethernet 4.1
sonic-cli(conf-if-Ethernet4.1)# ip igmp query-interval interval
```
...
### Syntax 
```
ip igmp query-max-response-time <querymaxrestime>
no ip igmp query-max-response-time
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
| querymaxrestime | IGMP version 1 to 250(Default: 10)  | Integer  |
### Usage Guidelines 
```
Use this command to configure IGMP.
```
...
```
sonic-cli(config)# interface Ethernet 4.1
sonic-cli(conf-if-Ethernet4.1)# ip igmp query-max-response-time time
```
## ip igmp snooping 
### Description 
```
...
| Name | Description | Type |
|:---:|:-----:|:-----:|
| version | IGMP version 1 or 2 or 3 (Default: 2)  | Integer  |
### Usage Guidelines 
```
Use this command to configure IGMP.
```
...
```
sonic-cli(config)# interface Ethernet 4.1
sonic-cli(conf-if-Ethernet4.1)# ip igmp version number
```
...
### Syntax 
```
ip load-share hash ipv4 { ipv4-src-ip | ipv4-dst-ip | ipv4-ip-proto | ipv4-l4-src-port | ipv4-l4-dst-port | symmetric }
...
### Syntax 
```
ip load-share hash ipv6 { ipv6-src-ip | ipv6-dst-ip | ipv6-next-hdr | ipv6-l4-src-port | ipv6-l4-dst-port | symmetric }
...
### Description 
```
Configure IP Route for Management VRF 
```
### Parent Commands (Modes) 
```
configure terminal
...
### Syntax 
```
ip route vrf mgmt <prefix> { { interface { <ifname> { { [ nexthop-vrf { <next-hop-vrf> { [ tag { <tag-val> [ <pref> ] } ] } [ <pref> ] } ] } | { [ tag { <tag-val> [ <pref> ] } ] } | [ <pref> ] ] } } } | { blackhole { [ tag { <tag-val> [ <pref> ] } ] } [ <pref> ] } | { <next-hop-addr> { { [ interface { <ifname> { { [ nexthop-vrf { <next-hop-vrf> { [ tag { <tag-val> { [ track { <trackid> [ <pref> ] } ] } [ <pref> ] } ] } [ <pref> ] { [ track { <trackid> [ <pref> ] } ] } } ] } | { [ track { <trackid> [ <pref> ] } ] } | { [ tag { <tag-val> { [ track { <trackid> [ <pref> ] } ] } [ <pref> ] } ] } | [ <pref> ] ] } } ] } | { [ tag { <tag-val> { [ track { <trackid> [ <pref> ] } ] } [ <pref> ] } ] } | [ <pref> ] | { [ track { <trackid> [ <pref> ] } ] } | { [ nexthop-vrf { <next-hop-vrf> { [ tag { <tag-val> { [ track { <trackid> [ <pref> ] } ] } [ <pref> ] } ] } [ <pref> ] { [ track { <trackid> [ <pref> ] } ] } } ] } ] } } }
no ip route vrf mgmt <prefix> { { interface { <ifname> { [ nexthop-vrf <next-hop-vrf> ] } } } | blackhole | { <next-hop-addr> { { [ interface { <ifname> { [ nexthop-vrf <next-hop-vrf> ] } } ] } | { [ nexthop-vrf <next-hop-vrf> ] } ] } } }
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
...
### Syntax 
```
ipv6 dhcp-relay source-interface { <intfName> | <pchName> | <vlanName> | <loName> }
...
```
sonic-cli(config)# interface Ethernet 0
sonic-cli(conf-if-Ethernet0)# ipv6 neighbor ip_addr mac
```
## ipv6 prefix-list 
### Description 
```
...
### Syntax 
```
ipv6 route vrf mgmt <prefix> { { interface { <ifname> { { [ nexthop-vrf { <next-hop-vrf> { [ tag { <tag-val> [ <pref> ] } ] } [ <pref> ] } ] } | { [ tag { <tag-val> [ <pref> ] } ] } | [ <pref> ] ] } } } | { blackhole { [ tag { <tag-val> [ <pref> ] } ] } [ <pref> ] } | { <next-hop-addr> { { [ interface { <ifname> { { [ nexthop-vrf { <next-hop-vrf> { [ tag { <tag-val> { [ track { <trackid> [ <pref> ] } ] } [ <pref> ] } ] } [ <pref> ] { [ track { <trackid> [ <pref> ] } ] } } ] } | { [ track { <trackid> [ <pref> ] } ] } | { [ tag { <tag-val> { [ track { <trackid> [ <pref> ] } ] } [ <pref> ] } ] } | [ <pref> ] ] } } ] } | { [ tag { <tag-val> { [ track { <trackid> [ <pref> ] } ] } [ <pref> ] } ] } | [ <pref> ] | { [ track { <trackid> [ <pref> ] } ] } | { [ nexthop-vrf { <next-hop-vrf> { [ tag { <tag-val> { [ track { <trackid> [ <pref> ] } ] } [ <pref> ] } ] } [ <pref> ] { [ track { <trackid> [ <pref> ] } ] } } ] } ] } } }
no ipv6 route vrf mgmt <prefix> { { interface { <ifname> { [ nexthop-vrf <next-hop-vrf> ] } } } | blackhole | { <next-hop-addr> { { [ interface { <ifname> { [ nexthop-vrf <next-hop-vrf> ] } } ] } | { [ nexthop-vrf <next-hop-vrf> ] } ] } } }
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
...
### Syntax 
```
ldap-server source-interface { Ethernet | Loopback | Management | PortChannel | Vlan | SubInterface }
...
### Syntax 
```
logging server <host> [ remote-port <vrport> ] [ source-interface { Ethernet | Loopback | Management | PortChannel | Vlan | SubInterface } ] [ vrf { mgmt | <vrf-name> } ]
...
### Syntax 
```
match interface { <phy-if-name> | PortChannel | { Vlan <vlan-id> } }
no match interface
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
| phy-if-name | EthernetX.Y  |   |
| vlan-id |   | Integer  |
...
### Syntax 
```
match peer { <match-peer> | <phy-if-name> | PortChannel | { Vlan <vlan-id> } }
no match peer
```
### Parameters 
| Name | Description | Type |
...
### Syntax 
```
max-metric { [ router-lsa { [ administrative ] | { [ on-startup <onstartuptimervalue> ] } | { [ all [ <maxmetricvalue> ] ] } | { [ include-stub [ <maxmetricvalue> ] ] } | { [ external-lsa { { [ all [ <maxmetricvalue> ] ] } | { [ connected [ <maxmetricvalue> ] ] } } ] } } { [ all [ <maxmetricvalue> ] ] } { [ include-stub [ <maxmetricvalue> ] ] } ] }
no max-metric { router-lsa { [ administrative ] | [ on-startup ] | [ all ] | [ include-stub ] | { external-lsa { [ all ] | [ connected ] } } } }
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
| onstartuptimervalue |   | Integer  |
...
| Name | Description | Type |
|:---:|:-----:|:-----:|
| session-name | String(Max: 24 characters)  | String  |
## mtu 
### Description 
```
Configure MTU 
...
### Syntax 
```
network <prefix> { [ route-map <route-map-name> ] }
no network <prefix> { [ route-map <route-map-name> ] }
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
...
### Syntax 
```
ntp server { <ip> | <server-name> } [ key <key-number> ] [ minpoll { <min-poll> { maxpoll <max-poll> } } ]
...
| key-number |   | Integer  |
| min-poll | seconds  | Integer  |
| max-poll | seconds  | Integer  |
### Usage Guidelines 
```
...
```
pbf next-hop-group <fbs-nhgrp-name> [ type { ip | ipv6 } ]
no pbf next-hop-group <fbs-nhgrp-name>
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
| fbs-nhgrp-name | WORD  | String  |
### Usage Guidelines 
```
...
```
### Examples 
#### Configures traffic class action for acl-copp Flow 
```
...
```
### Examples 
#### configure the primary IPv4 address 
```
sonic(config)# interface vxlan vtep1
...
### Syntax 
```
radius-server host <host> [ auth-port <vauth_port> ] [ auth-type <vauth_type> ] [ key { <vkey> [ encrypted ] } ] [ priority <vpriority> ] [ retransmit <vretransmit> ] [ source-interface { Ethernet | Loopback | Management | PortChannel | Vlan | SubInterface } ] [ timeout <vtimeout> ] [ vrf { mgmt | <vrf-name> } ]
...
### Syntax 
```
radius-server key <key> [ encrypted ]
...
### Syntax 
```
service-policy type { qos | monitoring | forwarding | copp | acl-copp } { in | out } <fbs-policy-name>
no service-policy type { qos | monitoring | forwarding | copp | acl-copp } { in | out }
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
...
### Syntax 
```
set ip { { next-hop <ip-address> { [ vrf { <vrf-name> | default } ] } } | { next-hop-group <pbf-nh-grp> } } [ priority <priority-value> ]
no set ip { { next-hop <ip-address> { [ vrf { <vrf-name> | default } ] } } | { next-hop-group <pbf-nh-grp> } } [ priority <priority-value> ]
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
...
no set ipv6 { { next-hop <ip-address> { [ vrf { <vrf-name> | default } ] } } | { next-hop-group <pbf-nh-grp> } } [ priority <priority-value> ]
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
...
### Syntax 
```
set trap-queue <trap-queue-id-value>
no set trap-queue
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
| trap-queue-id-value |   | Integer  |
### Examples 
#### Configures CPU Queue for acl-copp flows 
```
...
```
total number of in-memory-logging 
...
```
show in-memory-logging lines [ <lines> ]
...
```
show interface { { counters { [ rate ] | [ <phy-if-name> ] | [ <po-name> ] ] } } | { Eth [ <iface_num> ] } | { Ethernet [ <iface_num> ] } | <iface_range_num> | { PortChannel [ <po-id> ] } | { Management [ <mgmt-if-id> ] } | <vlan_range_num> | { Loopback <lo-id> } | status | { transceiver [ <phy-if-name> ] [ summary ] { [ dom ] | { [ diagnostics { capability | status } ] } ] } } }
...
```
show ip arp interface { { <phy-if-name> [ summary ] } | { <phy-subif-name> [ summary ] } | { Loopback { <lo-id> [ summary ] } } | { Management { <mgmt-if-id> [ summary ] } } | { PortChannel { <lag-id> [ summary ] } } | { Vlan { <vlan-id> [ summary ] } } | { Vxlan { <vxlan-if-name> [ summary ] } } }
...
```
show ip dhcp-relay { [ brief ] | { [ detailed { [ <intfName1> ] | [ <pchName1> ] | [ <vlanName1> ] } ] } | { [ statistics { [ <intfName> ] | [ <pchName> ] | [ <vlanName> ] } ] } }
...
| Name | Description | Type |
|:---:|:-----:|:-----:|
| intfName1 | EthernetX.Y  |   |
| pchName1 | PortChannelNUM  |   |
| vlanName1 | VlanNUM  |   |
| intfName | EthernetX.Y  |   |
| pchName | PortChannelNUM  |   |
| vlanName | VlanNUM  |   |
...
```
show ip igmp interface <interfacename> [ detail ]
...
```
### Examples 
#### show ip igmp interface Vlan301 
...
```
show ip igmp vrf { <vrf-name> | all } { { [ interface { [ <interfacename> [ detail ] ] } ] } | { [ statistics { [ interface <interfacename> ] } ] } | [ groups ] | [ sources ] | [ join ] } ]
...
```
show ip pim [ vrf { <vrf-name> | all } ] { { [ interface [ <ifName> ] ] } | { [ nbr [ <nbr-addr> ] ] } | [ rpf ] | [ ssm ] | { [ topology { [ <grp-addr> [ <src-addr> ] ] } ] } }
...
```
show ip route [ vrf { { all summary } | <vrfname> } ] { [ <address> ] | [ <prefix> ] | [ summary ] | [ bgp ] | [ connected ] | [ static ] | [ ospf ] } ]
...
```
show ipv6 access-lists [ <access-list-name> { { [ interface { Ethernet | <PortChannel> | <Vlan> | <eth-sub-if-id> | <po-sub-if-id> } ] } | [ Switch ] ] } ]
...
```
show ipv6 dhcp-relay { [ brief ] | { [ detailed { [ <intfName1> ] | [ <pchName1> ] | [ <vlanName1> ] } ] } | { [ statistics { [ <intfName> ] | [ <pchName> ] | [ <vlanName> ] } ] } }
...
```
show ipv6 neighbors interface { { [ <phy-if-name> [ summary ] ] } | { [ <subif-name> [ summary ] ] } | { Loopback { <lo-id> [ summary ] } } | { Management { <mgmt-if-id> [ summary ] } } | { PortChannel { <lag-id> [ summary ] } } | { Vlan { <vlan-id> [ summary ] } } | { Vxlan { <vxlan-if-name> [ summary ] } } }
...
```
show ipv6 route [ vrf { { all summary } | <vrfname> } ] { [ <address> ] | [ <prefix> ] | [ summary ] | [ bgp ] | [ connected ] | [ static ] } ]
...
```
show mac access-lists [ <access-list-name> { { [ interface { Ethernet | <PortChannel> | <Vlan> | <eth-sub-if-id> | <po-sub-if-id> } ] } | [ Switch ] ] } ]
...
| Name | Description | Type |
|:---:|:-----:|:-----:|
| show-fbs-group-name | WORD  | String  |
...
```
show pbf next-hop-group status Switch { [ <show-fbs-group-name> ] | [ type ] } ] { ip | ipv6 }
...
```
show pbf next-hop-group status interface { <eth-if-id> | <po-if-id> | <vlan-if-id> | <eth-sub-if-id> | <po-sub-if-id> } { [ <show-fbs-group-name> ] | [ type ] } ] { ip | ipv6 }
...
```
show policy-map { [ <show-fbs-policy-name> ] | [ type ] } ] { qos | monitoring | forwarding | copp | acl-copp }
...
```
show qos interface { CPU | { <phy-intf-name> { [ queue <queue-id> ] } { [ priority-flow-control { statistics [ queue ] } ] } } | <po-intf-name> | <vlan-intf-name> | <phy-sub-if-name> }
...
| phy-sub-if-name | Interface Type  |   |
...
```
Show current PBF next-hop groups configuration 
...
```
show running-configuration pbf next-hop-group [ <show-fbs-group-name> ]
...
```
show running-configuration subinterface { [ <subifname> ] | { [ PortChannel [ <pch_num> ] ] } } ]
...
```
### Examples 
#### Display all subinterface config 
...
```
#### Display specific subinterface config 
...
```
Show VRF configuration information 
...
```
show running-configuration vrf <vrf-name>
...
```
show service-policy { Switch | CtrlPlane } [ type { qos | monitoring | forwarding | copp | acl-copp } ]
...
```
show service-policy interface { <eth-if-id> | <po-if-id> | <vlan-if-id> | <eth-sub-if-id> | <po-sub-if-id> | CPU } [ type { qos | monitoring | forwarding | copp | acl-copp } ]
...
```
show service-policy policy-map <fbs-policy-name> { { [ interface { <eth-if-id> | <po-if-id> | <vlan-if-id> | <eth-sub-if-id> | <po-sub-if-id> | CPU } ] } | [ Switch ] | [ CtrlPlane ] } ]
...
```
show service-policy summary { { [ interface { <eth-if-id> | <po-if-id> | <vlan-if-id> | <eth-sub-if-id> | <po-sub-if-id> | CPU } ] } | [ Switch ] | [ CtrlPlane ] } ] [ type { qos | monitoring | forwarding | copp | acl-copp } ]
...
```
### Examples 
#### Display all subinterfaces information 
...
```
### Examples 
#### Shows route-scale switch resource information 
...
### Syntax 
```
source-interface { Ethernet | Subif | PortChannel | Vlan }
no source-interface
```
### Examples 
...
### Syntax 
```
switchport trunk allowed { Vlan { { add <vlan_id_list> } | { remove <vlan_id_list> } | { except <vlan_id_list> } | all | none | <vlan_id_list> } }
no switchport trunk allowed { Vlan <vlan_id_list> }
```
### Parameters 
| Name | Description | Type |
|:---:|:-----:|:-----:|
| vlan_id_list | <1..4094>  | String  |
### Usage Guidelines 
```
...
### Syntax 
```
tacacs-server host <host> [ port <port-val> ] [ timeout <timeout-val> ] [ key { <key-val> [ encrypted ] } ] [ type <type-val> ] [ priority <priority-val> ] [ vrf { mgmt } ]
...
### Syntax 
```
tacacs-server key <secret-key> [ encrypted ]
...
### Syntax 
```
threshold { { type { percentage | count } { [ up <threshold-up> ] } { [ down <threshold-down> ] } } | { up <threshold-up> { [ down <threshold-down> ] } } | { down <threshold-down> } }
...
name could be router port or Port Channel or Vlan interface or
routed Vlan sub-interface with IPv4/IPv6 address configured on it.
```
### Parent Commands (Modes) 
```
neighbor { <ip> | { interface { Ethernet | PortChannel | Vlan } } }
...
interface name could be router port or Port Channel or Vlan interface or
routed Vlan sub-interface with IPv4/IPv6 address configured on it.
```
### Parent Commands (Modes) 
```
...
| vrrp-id |   | Integer  |
### Examples 
#### Configure VRRP for IPv4 on sub-interface 
```
sonic(config)# interface Eth4.1
sonic(conf-if-Eth4.1)#
sonic(conf-if-Eth4.1)# vrrp 1 address-family ipv4
sonic(conf-if-Eth4.1-vrrp-ipv4-1)#
```
#### Configure VRRP for IPv6 on Ethernet interface 
```
sonic(config)# interface Eth4.1
sonic(conf-if-Eth4.1)#
sonic(conf-if-Eth4.1)# vrrp 1 address-family ipv6
```
## warm-reboot 
